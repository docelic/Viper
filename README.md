...
The basis of the whole system is an OpenLDAP server with a custom Viper backend, containing significantly extended functionality and all configuration data.
...
As part of Viper, a custom OpenLDAP backend has been implemented, and an important property of a custom backend became apparent &mdash; the ability to adapt to a wide range of existing software and their specifics of accessing LDAP, instead of possibly requiring modifications to their source code to glue it all together.

Finally, one of the design goals was to make Viper just feel right and easy to get results with. For that purpose, everything has been commented, with the availability of complete, working config files and this guide.
...
Viper "rewrites" incoming LDAP searches in numerous ways, completely avoiding all duplication of data in its hierarchy, while at the same time transparently handling all clients who issue their standard LDAP queries and expect to find information in their standard locations. (Or clients who don't even use LDAP searches but completely different methods, such as clients requesting preseed data over HTTP at the beginning of the machine installation).
...
One of Viper's design goals is to fully automate package installation and configuration. For standard Debconf questions, in non-Viper environments this is usually done by extracting the packages' Debconf question templates, answering them, and saving both to the Debconf database, so that the questions and answers are found when the client machines ask for them. However, extracting and answering Debconf questions ahead of time is quite a nuisance, especially when the packages or questions needed are not known ahead of time. Therefore, Viper automatically answers all known/existing questions, while all new/unseen questions can be opened as a popup window on the administrator's remote host of choice, where they can be answered from a central location and then saved to Debconf as expected for all future queries.

Furthermore, for each Debconf question asked (locally or remotely), an additional question is posed to the administrator, allowing the answer to be inserted in LDAP at the individual machine level (applying only to that machine), at the customer level (applying to all customer's machines), or at the global level (applying to all machines of all customers).

## Server Setup

Viper comes with the mentioned custom backend for OpenLDAP and the backend must be installed for Viper to work. This backend is the primary component of Viper and it implements multiple features that make Viper an extremely powerful tool for configuration management.
...
Therefore, all things considered, we advise to only run Viper on dedicated machines and inside local/trusted networks.
...
Here's some additional context and a summarized list of install steps:
